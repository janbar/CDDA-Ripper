/*
    SPDX-FileCopyrightText: 2005-2007 Richard Lärkäng <nouseforaname@home.se>
...
namespace KCDDB
{
  ...
  Result MusicBrainzLookup::lookup( const TrackOffsetList & trackOffsetList )
  {
            ...
            if (MediaList.NumItems() > 0)
            {
              ...
              for (int i=0; i < MediaList.NumItems(); i++)
              {
                ...
                if (FullRelease->MediumList()->NumItems() > 1)
                  title = tr("%1 (disc %2)").arg(title).arg(Medium->Position());
  ...
  QString MusicBrainzLookup::calculateDiscId(const TrackOffsetList & trackOffsetList )
  {
    ...
    sha.addData(QByteArray(temp, strlen(temp)));
