...
const QString SchemeParser::parseScheme(const QString &scheme, const Placeholders &placeholders, PlaceholdersParameters *placeholders_parameters)
{
        ...
        if (region_in_braces) {
            if (region_placeholdername_in_braces) {
                if (c.isLetterOrNumber() || c == QChar('_')) {
                    ...
                    p_error_string = tr("Illegal character found at index %1: '%2'").arg(index).arg(c);
                    return QString();
                }
    ...
    if (region_in_braces) {
        p_error_string = tr("Unclosed brace.");
        return QString();
    }
                                                  ...
                                                  const bool twodigitstracknum)
{
    ...
    placeholders.insert(VAR_APP, customize_placeholder_value(CDDARIPPER_NAME "-" CDDARIPPER_VERSION));
                                                 ...
                                                 const bool demomode)
{
    ...
    if (placeholders_found.contains(VAR_COVER_FILE)) {
        ...
        bool stop = false;
        if (demomode) {
            filename = tmppath + "cover.123." + format.toLower();
            ...
            if ((!finfo.exists()) && (!stop)) {
                ...
                if ((!cover) || ((cover) && (cover->isEmpty()))) {
                    if (cover_parameters.contains("usenocover")) {
                        QImage c = QImage(QString(":/images/nocover.png"));
                        if ((x != -1) && (y != -1)) {
        ...
        if (!stop) {
            placeholders.insert(VAR_COVER_FILE, filename);
        }
    }
...
const QString SchemeParser::parseSimpleFilenameScheme(const QString &text,
                                                const int cdno,
                                                const int nooftracks,
                                                const QString &artist,
                                                const QString &title,
                                                ...
                                                const bool fat32compatible)
{
    ...
    placeholders.insert(VAR_ALBUM_ARTIST, customize_placeholder_value(artist, fat32compatible, true));
    placeholders.insert(VAR_ALBUM_TITLE, customize_placeholder_value(title, fat32compatible, true));
    placeholders.insert(VAR_DATE, customize_placeholder_value(date, fat32compatible, true));
    placeholders.insert(VAR_GENRE, customize_placeholder_value(genre, fat32compatible, true));
                                         ...
                                         const int nooftracks)
{
    ...
    placeholders.insert(VAR_APP, CDDARIPPER_NAME "-" CDDARIPPER_VERSION);
