...
const QString CachedImage::supportedMimeTypeList()
{
    if (mime_cache.isEmpty()) {
        ...
        QString result = "Images(*.jpg *.jpeg *.png *.gif)";
...
bool CachedImage::load(const QString &filename)
{
    QFile file(filename);
    if (!file.exists()) {
        _error = Error(QObject::tr("File does not exist."), QObject::tr("Please check if the file really exists."), Error::ERROR);
        return false;
    }
    ...
    if (!_format.isEmpty() && QImageReader::supportedImageFormats().contains(_format)) {
        if (!file.open(QIODevice::ReadOnly)) {
            _error = Error(QObject::tr("Cannot open file."), QObject::tr("Please check your file. Maybe you do not have proper permissions."), Error::ERROR);
            return false;
        }
        _data = file.readAll();
        file.close();
    } else {
        _error = Error(QObject::tr("Unsupported image format"), QObject::tr("Please check your file. Maybe it is corrupted. Otherwise try to convert your cover image with an external program to a common file format like JPEG."), Error::ERROR);
        _format = "";
        _size = QSize();
        return false;
    }
...
bool CachedImage::save(const QString &filename, const QSize &size)
{
    ...
    if (!file.open(QIODevice::WriteOnly)) {
        _error = Error(QObject::tr("Cannot open file"), QObject::tr("Please check your permissions."), Error::ERROR);
        return false;
    }
    QMimeDatabase db;
    ...
    if ((compare_format(format, _format) || (format.isEmpty())) && ((size.isNull()) || (size == _size))) {
        ...
        if ((r == -1) || (r < _data.size())) {
            _error = Error(QObject::tr("Cannot save file"), QObject::tr("Please check your permissions. Do you have enough space?"), Error::ERROR);
            file.close();
            return false;
        }
    } else {
...
bool CachedImage::_save(QIODevice *device, const QByteArray &format, const QSize &size)
{
    if ((!format.isEmpty()) && (!QImageReader::supportedImageFormats().contains(format))) {
        _error = Error(QObject::tr("Unsupported image format"), QObject::tr("Please use common image formats like JPEG, PNG or GIF as they are supported on almost all systems."), Error::ERROR);
        return false;
    }
    ...
    if (!ok) {
        _error = Error(QObject::tr("Cannot write on device"), QObject::tr("Please check your permissions. Do you have enough space on your device?"), Error::ERROR);
        return false;
    }
    _error = Error();
    return true;
