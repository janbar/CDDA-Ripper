    ...
    : QThread(parent)
{
    ...
    if (!paranoia) {
        ...
        Q_EMIT error(tr("Internal device error."), tr("Check your device and make a bug report."));
        return;
    }
...
void CDDAExtractThread::run()
{
    ...
    if (first_sector < 0 || last_sector < 0) {
        Q_EMIT info(tr("Extracting finished."));
        return;
    }
    ...
    if (track > 0) {
        ...
        Q_EMIT info(tr("Ripping track %1 (%2:%3)...").arg(track).arg(min).arg(sec));
    } else {
        Q_EMIT info(tr("Ripping whole CD as single track."));
    }
    extract_protocol.append(tr("Start reading track %1 with %2 sectors").arg(track).arg(sectors_all));
    ...
    if (b_interrupt)
        Q_EMIT error(tr("User canceled extracting."));

    if (b_error)
        Q_EMIT error(tr("An error occurred while ripping track %1.").arg(track));

    if ((!b_interrupt) && (!b_error)) {
        if (track > 0) {
            Q_EMIT info(tr("Ripping OK (Track %1).").arg(track));
        } else {
            Q_EMIT info(tr("Ripping OK."));
        }
    }

    qDebug() << "Reading finished.";
    extract_protocol.append(tr("Reading finished"));
}
...
void CDDAExtractThread::createStatus(long sector, int status)
{
    ...
    case PARANOIA_CB_FIXUP_EDGE:
        ...
        extract_protocol.append(tr("Fixed edge jitter (absolute sector %1, relative sector %2, track time pos %3:%4)").arg(sector).arg(current_sector).arg(tp_min).arg(tp_sec));
        break;
    case PARANOIA_CB_FIXUP_ATOM:
        qDebug() << "Fixed atom jitter";
        extract_protocol.append(tr("Fixed atom jitter (absolute sector %1, relative sector %2, track time pos %3:%4)").arg(sector).arg(current_sector).arg(tp_min).arg(tp_sec));
        break;
    case PARANOIA_CB_SCRATCH:
        ...
        if (!scratch_detected) {
            ...
            Q_EMIT warning(tr("Scratch detected (absolute sector %1, relative sector %2, track time pos %3:%4)").arg(sector).arg(current_sector).arg(tp_min).arg(tp_sec));
        }
        extract_protocol.append(tr("SCRATCH DETECTED (absolute sector %1, relative sector %2, track time pos %3:%4)").arg(sector).arg(current_sector).arg(tp_min).arg(tp_sec));
        break;
    case PARANOIA_CB_REPAIR:
        qDebug() << "Repair";
        extract_protocol.append(tr("Repair (absolute sector %1, relative sector %2, track time pos %3:%4)").arg(sector).arg(current_sector).arg(tp_min).arg(tp_sec));
        break;
    case PARANOIA_CB_SKIP:
        // skipped sector
        qDebug() << "Skip";
        Q_EMIT warning(tr("Skip sectors (absolute sector %1, relative sector %2, track time pos %3:%4)").arg(sector).arg(current_sector).arg(tp_min).arg(tp_sec));
        extract_protocol.append(tr("SKIP (absolute sector %1, relative sector %2, track time pos %3:%4)").arg(sector).arg(current_sector).arg(tp_min).arg(tp_sec));
        break;
    case PARANOIA_CB_DRIFT:
        qDebug() << "Drift";
        extract_protocol.append(tr("Drift (absolute sector %1, relative sector %2, track time pos %3:%4)").arg(sector).arg(current_sector).arg(tp_min).arg(tp_sec));
        break;
    case PARANOIA_CB_BACKOFF:
        qDebug() << "Backoff";
        extract_protocol.append(tr("Backoff (absolute sector %1, relative sector %2, track time pos %3:%4)").arg(sector).arg(current_sector).arg(tp_min).arg(tp_sec));
        break;
    ...
    case PARANOIA_CB_FIXUP_DROPPED:
        ...
        extract_protocol.append(tr("Fixup dropped (absolute sector %1, relative sector %2, track time pos %3:%4)").arg(sector).arg(current_sector).arg(tp_min).arg(tp_sec));
        break;
    case PARANOIA_CB_FIXUP_DUPED:
        qDebug() << "Fixup duped";
        extract_protocol.append(tr("Fixup duped (absolute sector %1, relative sector %2, track time pos %3:%4)").arg(sector).arg(current_sector).arg(tp_min).arg(tp_sec));
        break;
    case PARANOIA_CB_READERR:
        qDebug() << "Read error";
        if (!read_error) {
            read_error = true;
            Q_EMIT warning(tr("Read error detected (absolute sector %1, relative sector %2, track time pos %3:%4)").arg(sector).arg(current_sector).arg(tp_min).arg(tp_sec));
        }
        extract_protocol.append(tr("READ ERROR (absolute sector %1, relative sector %2, track time pos %3:%4)").arg(sector).arg(current_sector).arg(tp_min).arg(tp_sec));
        break;
    }
}
