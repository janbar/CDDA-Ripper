    ...
    : QAbstractTableModel(parent)
{
    ...
    if (!devices) {
        ...
        error = Error(tr("Unable to create devices object."), tr("This is an internal error. Check your hardware. If all okay please make bug report."), Error::ERROR, this);
        return;
    }
    ...
    if (!cddb) {
        ...
        error = Error(tr("Unable to create KCDDB object."), tr("This is an internal error. Check your hardware. If all okay please make bug report."), Error::ERROR, this);
        return;
    }
    ...
    if ((role == Qt::DisplayRole) || (role == Qt::CheckStateRole && index.column() == CDDA_MODEL_COLUMN_RIP_INDEX) || (role == CDDA_MODEL_INTERNAL_ROLE) || (role == Qt::EditRole)) {
        ...
        case CDDA_MODEL_COLUMN_TITLE_INDEX:
            if (isAudioTrack(index.row() + 1)) {
                ...
                if (t.isEmpty())
                    return tr("Track %1").arg(index.row() + 1);
                return t;
            }
            break;
        default:;
...
#else
    QRegularExpression rx1("[\\(|\\[]* *([c|C][d|D]|[d|D][i|I][s|S][k|c|K|C]) *[0-9]* *[\\)|\\]]* *$");
    QRegularExpressionMatch match = rx1.match(t);
    if (match.hasMatch()) {
        int i = match.capturedStart(0);
        QString frac = t.mid(i);
        ...
        {
            bool ok;
            int cdnum = match2.captured(1).toInt(&ok);
            if (ok) {
                if (cdnum < 0)
                    return -1;
                if (cdnum == 0)
...
void CDDAModel::new_audio_disc_available(const QString &udi)
{
    ...
    if (!pn) {
        ...
        error = Error(tr("Unable to create CDDA paranoia object."), tr("This is an internal error. Check your hardware. If all okay please make bug report."), Error::ERROR, this);
        return;
    }
...
void CDDAModel::lookup_cddb_done(KCDDB::Result result)
{
    if ((result != KCDDB::Success) && (result != KCDDB::MultipleRecordFound)) {
        ...
        case KCDDB::ServerError:
            error = Error(KCDDB::resultToString(result), tr("There is an error with the CDDB server. Please wait or contact the administrator of the CDDB server."), Error::ERROR, this);
            break;
        case KCDDB::HostNotFound:
            error = Error(KCDDB::resultToString(result), tr("Cannot find the CDDB server. Check your network. Maybe the CDDB server is offline."), Error::ERROR, this);
            break;
        case KCDDB::NoResponse:
            error = Error(KCDDB::resultToString(result), tr("Please wait, maybe the server is busy, or contact the CDDB server administrator."), Error::ERROR, this);
            break;
        case KCDDB::InvalidCategory:
            error = Error(KCDDB::resultToString(result), tr("This should not happen. Please make a bug report."), Error::ERROR, this);
            break;
        case KCDDB::UnknownError:
            error = Error(KCDDB::resultToString(result), tr("Please make a bug report and contact the CDDB server administrator."), Error::ERROR, this);
            break;
        case KCDDB::NoRecordFound:;
        case KCDDB::MultipleRecordFound:;
        case KCDDB::Success:;
        default:
            error = Error(KCDDB::resultToString(result), tr("This means no data found in the CDDB database."), Error::ERROR, this);
        }
        cddb_transaction_pending = false;
    ...
    if (cddb->lookupResponse().count() > 1) {
        ...
        QString res = QInputDialog::getItem(nullptr, tr("Select CDDB Entry"), tr("Select a CDDB entry:"), list, 0, false, &ok, {});
