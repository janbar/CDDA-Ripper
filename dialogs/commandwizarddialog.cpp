    ...
    : QDialog(parent)
{
    ...
    setWindowTitle(tr("Command Scheme Wizard"));
    ...
    connect(ui.kurllabel_aboutcommandlineschemes, SIGNAL(linkActivated(const QString &)), this, SLOT(about_commandline_schemes()));
    connect(ui.kurllabel_aboutparameters, SIGNAL(linkActivated(const QString &)), this, SLOT(about_parameters()));
...
void CommandWizardDialog::about_commandline_schemes()
{
    QWhatsThis::showText(
        ...
        tr("<p>The following variables will be replaced with their particular meaning in every track name.</p>"
             "<p><table border=\"1\">"
             ...
             "<tr><td>$i</td><td>The temporary WAV file (input file) created by CDDA-Ripper from CD audio track. You can use it as a normal input file for your command line encoder.</td></tr>"
             ...
             "<tr><td>$app</td><td>Application name and version.</td></tr>"
             "</table></p>"),
...
void CommandWizardDialog::about_parameters()
{
    QWhatsThis::showText(ui.kurllabel_aboutparameters->mapToGlobal(ui.kurllabel_aboutparameters->geometry().topLeft()),
                         tr("<p>Variables in CDDA-Ripper can have parameters. E.g.</p>"
