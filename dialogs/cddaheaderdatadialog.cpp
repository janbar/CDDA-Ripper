    ...
    : QDialog(parent)
{
    ...
    ui.kcombobox_genre->setCurrentText(cdda_model->genre());
    connect(ui.kcombobox_genre, SIGNAL(currentTextChanged(const QString &)), this, SLOT(trigger_changed()));
    {
        bool ok;
    ...
    ui.qlineedit_freedbdiscid->setText(QString("0x%1").arg(DiscIDCalculator::CDDBId(cdda_model->discSignature()), 0, 16));
...
void CDDAHeaderDataDialog::save()
{
    ...
    cdda_model->setGenre(ui.kcombobox_genre->currentText());
...
void CDDAHeaderDataDialog::trigger_changed()
{
    ...
    if (ui.kcombobox_genre->currentText() != cdda_model->genre()) {
        applyButton->setEnabled(true);
        return;
    }
