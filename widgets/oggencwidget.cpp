...
void oggencWidget::trigger_changed()
{
    changed = (kdoublenuminput_quality->value() != parameters->value(ENCODER_OGGENC_QUALITY_KEY, ENCODER_OGGENC_QUALITY).toReal() ||
               kintspinbox_minbitrate->value() != parameters->value(ENCODER_OGGENC_MINBITRATE_VALUE_KEY, ENCODER_OGGENC_MINBITRATE_VALUE).toInt() || checkBox_minbitrate->isChecked() != parameters->value(ENCODER_OGGENC_MINBITRATE_KEY).toBool() ||
               kintspinbox_maxbitrate->value() != parameters->value(ENCODER_OGGENC_MAXBITRATE_VALUE_KEY, ENCODER_OGGENC_MAXBITRATE_VALUE).toInt() || checkBox_maxbitrate->isChecked() != parameters->value(ENCODER_OGGENC_MAXBITRATE_KEY).toBool() ||
