...
bool Audex::prepare()
{
    if (profile_model->currentProfileIndex() < 0) {
        slot_error(tr("No profile selected. Operation abort."));
        return false;
    }
...
void Audex::start()
{
    ...
    Q_EMIT info(tr("Start ripping and encoding with profile \"%1\"...").arg(p_profile_name));
    if (check())
        start_extract();
    else
    ...
    if (p_single_file) {
        ...
        if (!skip) {
            ...
            if (!targetFilename.isEmpty()) {
                ...
                QString sourceFilename = tmp_path + QString("%1").arg(DiscIDCalculator::CDDBId(cdda_model->discSignature())) + '.' + QString("%1").arg(ex_track_index) + ".wav";
                ex_track_source_filename = sourceFilename;
                wave_file_writer->open(sourceFilename);
                                      ...
                                      bool is_first_track)
{
    ...
    if (lastSlash == -1) {
        Q_EMIT error(tr("Can't find path \"%1\".").arg(targetFilename), tr("Please check your path (write access?)"));
        return false;
    }
    ...
    if (free < 200 * 1024) {
        Q_EMIT warning(tr("Free space on \"%1\" is less than 200 MiB.").arg(targetPath));
    }

    auto *file = new QFile(targetFilename);
    if (file->exists()) {
        if (overwrite_existing_files) {
            Q_EMIT warning(tr("Warning! File \"%1\" already exists. Overwriting.").arg(targetStrippedFilename));
        } else {
            Q_EMIT warning(tr("Warning! File \"%1\" already exists. Skipping.").arg(targetStrippedFilename));
                                                     ...
                                                     bool overwrite_existing_files)
{
    SchemeParser schemeparser;
    targetFilename = ((basepath.right(1) == "/") ? basepath : basepath + "/")
        + schemeparser.parseSimpleFilenameScheme(
    ...
    if (!dir->exists()) {
        if (!dir->mkpath(targetPath)) {
            Q_EMIT error(tr("Unable to create folder \"%1\".").arg(targetPath), tr("Please check your path (write access?)"));
            return false;
        } else {
            Q_EMIT info(tr("Folder \"%1\" successfully created.").arg(targetPath));
        }
    } else {
        Q_EMIT warning(tr("Folder \"%1\" already exists.").arg(targetPath));
    }
    delete dir;
    ...
    quint64 free = diskfreespace.bytesAvailable() / 1024;
    if (free < 800 * 1024) {
        Q_EMIT warning(tr("Free space on \"%1\" is less than 800 MiB.").arg(targetPath));
    }
...
void Audex::execute_finish()
{
    ...
    if ((_finished_successful) && (profile_model->data(profile_model->index(profile_model->currentProfileRow(), PROFILE_MODEL_COLUMN_SC_INDEX)).toBool())) {
        ...
        if (!cdda_model->isCoverEmpty()) {
            ...
            SchemeParser schemeparser;
            QString filename = schemeparser.parseSimpleFilenameScheme(
                scheme,
                cdda_model->cdNum(),
            ...
            if (p_prepare_dir(filename, target_dir, overwrite)) {
                if (image.save(filename, format.toLatin1().data())) {
                    Q_EMIT info(tr("Cover \"%1\" successfully saved.").arg(QFileInfo(filename).fileName()));
                    co = filename;
                } else {
                    Q_EMIT error(tr("Unable to save cover \"%1\".").arg(QFileInfo(filename).fileName()), tr("Please check your path and permissions"));
                }
            }
        }
    }
    ...
    if ((_finished_successful) && (profile_model->data(profile_model->index(profile_model->currentProfileRow(), PROFILE_MODEL_COLUMN_PL_INDEX)).toBool()) && (target_filename_list.count() > 0) && (!p_single_file)) {
        ...
        if (p_prepare_dir(filename, target_dir, (overwrite && !cdda_model->isMultiCD() && (cdda_model->cdNum() < 1)))) {
            ...
            if (file.open(QFile::WriteOnly | QFile::Truncate)) {
                ...
                file.close();
                Q_EMIT info(tr("Playlist \"%1\" successfully created.").arg(QFileInfo(filename).fileName()));
                pl = filename;

            } else {
                Q_EMIT error(tr("Unable to save playlist \"%1\".").arg(QFileInfo(filename).fileName()), tr("Please check your path and permissions"));
            }
        }
    }
    ...
    if ((_finished_successful) && (profile_model->data(profile_model->index(profile_model->currentProfileRow(), PROFILE_MODEL_COLUMN_INF_INDEX)).toBool())) {
        ...
        if (p_prepare_dir(filename, target_dir, overwrite)) {
            ...
            if (file.open(QFile::WriteOnly | QFile::Truncate)) {
                ...
                out << text.join("\n");
                file.close();
                Q_EMIT info(tr("Info file \"%1\" successfully created.").arg(QFileInfo(filename).fileName()));
                in = filename;
            } else {
                Q_EMIT error(tr("Unable to save info file \"%1\".").arg(QFileInfo(filename).fileName()), tr("Please check your path and permissions"));
            }
        }
    }
    ...
    if ((_finished_successful) && (profile_model->data(profile_model->index(profile_model->currentProfileRow(), PROFILE_MODEL_COLUMN_HL_INDEX)).toBool())
        ...
        if (p_prepare_dir(filename, target_dir, (overwrite && !cdda_model->isMultiCD() && (cdda_model->cdNum() < 1)))) {
            ...
            if (success) {
                ...
                file.close();
                Q_EMIT info(tr("Hashlist \"%1\" successfully created.").arg(QFileInfo(filename).fileName()));
                hl = filename;
            } else {
                Q_EMIT error(tr("Unable to save hashlist \"%1\".").arg(QFileInfo(filename).fileName()), tr("Please check your path and permissions"));
            }
        }
    }
    ...
    if ((_finished_successful) && (profile_model->data(profile_model->index(profile_model->currentProfileRow(), PROFILE_MODEL_COLUMN_CUE_INDEX)).toBool())
        ...
        if (p_prepare_dir(filename, target_dir, overwrite)) {
            ...
            if (success) {
                ...
                file.close();
                Q_EMIT info(tr("Cue sheet \"%1\" successfully created.").arg(QFileInfo(filename).fileName()));
                cs = filename;
            } else {
                Q_EMIT error(tr("Unable to save cue sheet \"%1\".").arg(QFileInfo(filename).fileName()), tr("Please check your path and permissions"));
            }
        }
    }
...
bool Audex::p_prepare_dir(QString &filename, const QString &targetDirIfRelative, const bool overwrite)
{
    ...
    if (fileinfo.isAbsolute()) {
        ...
        if (!targetDirIfRelative.isEmpty()) {
            ...
            if (!dir.isReadable()) {
                Q_EMIT error(tr("Unable to open folder \"%1\".").arg(targetDirIfRelative), tr("Please check your path and permissions"));
                return false;
            }
    ...
    if (!overwrite) {
        ...
        if (info.exists()) {
            Q_EMIT warning(tr("Warning! File \"%1\" already exists. Skipping.").arg(info.fileName()));
            return false;
        }
    }
...
bool Audex::p_mkdir(const QString &absoluteFilePath)
{
    ...
    if (dir.exists()) {
        if (!dir.isReadable()) {
            Q_EMIT error(tr("Unable to open folder \"%1\".").arg(absoluteFilePath), tr("Please check your path and permissions"));
            return false;
        }
    } else {
        if (!dir.mkpath(absoluteFilePath)) {
            Q_EMIT error(tr("Unable to create folder \"%1\".").arg(absoluteFilePath), tr("Please check your path (write access?)"));
            return false;
        } else {
            Q_EMIT info(tr("Folder \"%1\" successfully created.").arg(absoluteFilePath));
        }
    }
