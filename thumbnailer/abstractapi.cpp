...
{
  QMap<QString, AbstractAPI*>::iterator it = apis.map.find(apiName);
...
AbstractArtistInfo::AbstractArtistInfo(const QString& artist, int fetchCount)
: m_artist(artist)
, m_fetchCount(fetchCount)
{
}

AbstractAlbumInfo::AbstractAlbumInfo(const QString& artist, const QString& album, int fetchCount)
: m_artist(artist)
, m_album(album)
, m_fetchCount(fetchCount)
{
