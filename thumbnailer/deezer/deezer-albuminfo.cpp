...
DEEZERAlbumInfo::DEEZERAlbumInfo(const QString& artist, const QString& album, int fetchCount)
: AbstractAlbumInfo(artist, album, fetchCount)
{
}
...
AbstractAPI::Parse_Status DEEZERAlbumInfo::parseInfo(const QByteArray& info, QList<AbstractAlbumInfo::metadata_t>& metas)
{
  if (info.isEmpty())
  ...
  if (n_data.IsArray())
  {
    for (size_t i = 0; i < n_data.Size(); ++i)
    {
      ...
      {
        AbstractAlbumInfo::metadata_t meta;
        ...
        if (--m_fetchCount < 1)
          break;
        qWarning("%s : fetchcount = %d", __FUNCTION__, m_fetchCount);
      }
    }
