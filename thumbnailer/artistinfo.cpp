...
ArtistInfo::ArtistInfo(DiskCacheManager* cache, NetManager* nam, AbstractAPI* api, const QString& artist, const QSize& requestedSize, int fetchCount, bool cached, QObject* parent)
...
void ArtistInfo::run()
{
  ...
  m_p = m_api->newArtistInfo(m_artist, m_fetchCount);
  queryInfo();
}
...
const QList<QByteArray>& ArtistInfo::images() const
{
  return m_images;
}
...
void ArtistInfo::processInfo()
{
  ...
  default:
  {
    ...
    for (const AbstractArtistInfo::metadata_t& meta : m_metas)
    {
    ...
    else
    {
      ...
      m_error.errorCode = 0;
      m_error.errorString = QString::fromUtf8(ERRMSG_NOT_FOUND " for artist=[%1] size=%2").arg(m_artist).arg(m_size);
      fakeImage();
      Q_EMIT finished();
    }
  }
...
AbstractAPI::Parse_Status ArtistInfo::parseInfo()
{
